...
def __(mo):
    ...
    *Professor Yunzhi Wang*
    ...
    Each phase (alpha and beta) forms its own 3D surface. The 2D plots you see are horizontal slices through these surfaces at constant temperature.
...
def __(
    ...
    ax_3d.plot(x_line, T_line, G_alpha_line, 'b-', linewidth=3, label='alpha phase (current T)')
    ax_3d.plot(x_line, T_line, G_beta_line, 'r-', linewidth=3, label='beta phase (current T)')
    ...
    # Plot the curves
    ax_2d.plot(x_range, G_alpha_vals, 'b-', linewidth=2, label='alpha phase')
    ax_2d.plot(x_range, G_beta_vals, 'r-', linewidth=2, label='beta phase')
    ...
    if valid_temps:
        ...
        ax_phase.plot(x1_values, valid_temps, 'b-', linewidth=2, label='alpha phase boundary')
        ax_phase.plot(x2_values, valid_temps, 'r-', linewidth=2, label='beta phase boundary')
        
        # Fill the two-phase region
        ax_phase.fill_betweenx(valid_temps, x1_values, x2_values, alpha=0.3, color='gray', label='alpha + beta')
        
        # Add labels for single-phase regions
        if x1_values:
            ax_phase.text(0.1, np.mean(valid_temps), 'alpha', fontsize=14, ha='center', style='italic')
            ax_phase.text(0.9, np.mean(valid_temps), 'beta', fontsize=14, ha='center', style='italic')
...
def __(mo, tangent_found, x1_tangent, x2_tangent):
    ...
    {f"- alpha phase composition: x_B = {x1_tangent:.3f}" if tangent_found else ""}
    {f"- beta phase composition: x_B = {x2_tangent:.3f}" if tangent_found else ""}
